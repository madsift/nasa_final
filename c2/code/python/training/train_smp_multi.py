...
# Import your modules - use package paths when running from code/ root
from python.data.datasets import CraterDataset3, get_valid_transforms, get_train_transforms
from python.models.models_smp import CraterSMP, CraterSMP_3Ch_RAM
...
def get_datasets(data_dir, tile_size=None):
...
def train_worker(rank, world_size, args):
    ...
    # Save directory
    save_dir = f'./{args.model}_{args.backbone}_3ch_ddp'
    ...
    # Create datasets
    train_ds, val_ds, n_train, n_val = get_datasets(args.data_dir, tile_size=tile_size)
    ...
    if args.model == "CraterSMP_3Ch_RAM":
        ...
        model = CraterSMP(backbone=args.backbone, num_classes=3)
    ...
    loss_fn = AutomaticWeightedLoss_Boundary(num_losses=3, use_global=True).cuda(rank)
...
def main():
    ...
    parser.add_argument("--model", type=str, default="CraterSMP_3Ch_RAM", help="Model to use")
    # Model settings
    parser.add_argument("--backbone", type=str, default="mobileone_s2")
    parser.add_argument("--data_dir", type=str, default="./train_tiles")
    parser.add_argument("--tile_size", type=str, default="544 416", help="Tile size as 'W H' for rectangular tile handling")
