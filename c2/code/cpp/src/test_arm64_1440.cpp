...
const int MAX_TILE_W = 736;  // Maximum tile width
const int MAX_TILE_H = 576;  // Maximum tile height
...
const int MAX_IMG_W = 1440;
const int MAX_IMG_H = 1140;
...
Args parse_args(int argc, char* argv[]) {
    ...
    if (args.tile_w > 0 && args.overlap_w == 0) {
        if (args.tile_w == 544 && args.tile_h == 416 && args.input_res == 1024) {
            ...
            args.overlap_w = 24;
            args.overlap_h = 32;
        } else if (args.tile_w == 736 && args.tile_h == 576 && args.input_res == 1440) {
            // Configuration: 736x576 tiles with 1440 input_res (1440x1140 padded image)
            args.overlap_w = 16;
            args.overlap_h = 6;
        } else {
...
int main(int argc, char* argv[]) {
    ...
    for (int img_idx = 0; img_idx < total_images; ++img_idx) {
        ...
        // === ADD PADDING FOR CLEAN TILING ===
        // For 1440 input_res: 2592x2048 -> 1440x1136.84, need 1440x1140 for 736x576 tiles
        // Pad bottom by 4 pixels: (2048+4)/1.8 ≈ 1140 (exact for integer tiling)
        int pad_bottom = 0;
        if (args.input_res == 1440 && args.tile_w == 736 && args.tile_h == 576) {
            // Target: 1140 height for clean 2x tiling with 576 tiles (overlap 6)
            // stride_h = 576 - 6 = 570, 570 * 2 = 1140
            int target_h = 1140;
            if (resized_h < target_h) {
                pad_bottom = target_h - resized_h;
                cv::copyMakeBorder(img_resized, img_resized, 0, pad_bottom, 0, 0, cv::BORDER_REPLICATE);
                resized_h = target_h;
            }
        }
        ...
        // === STRIP PADDING BEFORE UPSCALE ===
        // If we added padding during resize, strip it from the output
        if (args.input_res == 1440 && args.tile_w == 736 && args.tile_h == 576) {
            // Calculate how many pixels of padding were added
            // Original resize: h_orig * scale_factor = resized_h_unpadded
            int resized_h_unpadded = (int)std::round(h_orig * scale_factor);
            for (int c = 0; c < OUT_CLASSES; ++c) {
                // Crop to remove padding (keep only top resized_h_unpadded rows)
                tiled_output[c] = tiled_output[c](cv::Rect(0, 0, tiled_output[c].cols, resized_h_unpadded)).clone();
            }
        }
