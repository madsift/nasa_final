...
def main(args):
    ...
    # Use opset 17 - compatible with ORT 1.16.x
    opset = 17
    ...
    # CRITICAL: Force IR version 9 for ORT 1.16.x compatibility
    # PyTorch 2.x exports IR version 10 which ORT 1.16 doesn't support
    onnx_model = onnx.load(onnx_path)
    if onnx_model.ir_version > 9:
        print(f"Downgrading IR version from {onnx_model.ir_version} to 9")
        onnx_model.ir_version = 9
        onnx.save(onnx_model, onnx_path)
    
    print(f"Exported to {onnx_path} (opset={opset}, ir_version=9)")
